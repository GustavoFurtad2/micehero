...
local function Player(name)
    ...
    function Player:LoadSongsList() 
        for id, song in next, SONGLIST do
            if id <= 2 then
	              ui.addTextArea(id, string.format("<font color='#%s'><a href='event:%s'>%s</a>", ALBUMS[SONGLIST[id].album].color, song.name, song.name), name, 10, 10 + (id * 50), 300, 25, 0xf, 0xf, 1, true)
            else
